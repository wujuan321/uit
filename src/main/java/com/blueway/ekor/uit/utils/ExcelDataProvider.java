...
public class ExcelDataProvider implements Iterator<Object[]> {
	...
	public ExcelDataProvider(String moduleName, String caseNum) {
		...
		try {
			...
			LoggerUtils.error("没有找到指定的文件：" + "[" + path + "]");
			Assert.fail("没有找到指定的文件：" + "[" + path + "]");
		} catch (Exception e) {
			LoggerUtils.error("不能读取文件： [" + path + "]", e);
	...
	// Iterator<Object[]> excel = new ExcelDataProvider("login", "001");
	// while (excel.hasNext()) {
	// Object[] obj = excel.next();
	// for (int i = 0; i < obj.length; i++) {
	// Map<String, String> data = (Map<String, String>) obj[i];
	// System.out.println(data.get("USERNAME") + "	"
	// + data.get("PASSWORD") + "	" + data.get("code"));
	// }
	// System.out.println();
	// }
	// }
